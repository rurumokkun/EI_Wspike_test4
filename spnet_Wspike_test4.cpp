...
const	int		NUM_EI_PATTERN = 193;	//				//EIの組み合わせの数  181031 10->1
const   int             num_trial=1;             //171101追記  1214に10->1へテスト用                     181031 20->1			 
...
float	v[Ngr][N], u[Ngr][N];				// activity variable
//float   ve[Ngr][N],vi[Ngr][N];
...
int	EI_1,EI_2;
int	num;
int               Ne[Ngr][NUM_EI_PATTERN]={{700,700,700,700,700,700,700,700,700,700,700,700,700,700,700,700,700,700,700,710,710,710,710,710,710,710,710,710,710,710,710,710,710,710,710,710,710,720,720,720,720,720,720,720,
  720,720,720,720,720,720,720,720,720,720,730,730,730,730,730,730,730,730,730,730,730,730,730,730,730,730,740,740,740,740,740,740,740,740,740,740,740,740,740,740,740,750,750,750,
  750,750,750,750,750,750,750,750,750,750,750,760,760,760,760,760,760,760,760,760,760,760,760,760,770,770,770,770,770,770,770,770,770,770,770,770,780,780,780,780,780,
  780,780,780,780,780,780,790,790,790,790,790,790,790,790,790,790,810,810,810,810,810,810,810,810,810,820,820,820,820,820,820,820,820,820,830,830,830,830,830,830,830,
  840,840,840,840,840,840,850,850,850,850,850,860,860,860,860,870,870,870,880,880,890},     { 710,720,730,740,750,760,770,780,790,810,820,830,840,850,860,870,880,890,900,720,730,740,750,760,770,780,790,810,820,830,840,850,860,870,880,890,900,730,740,750,760,770,780,790,
  810,820,830,840,850,860,870,880,890,900,740,750,760,770,780,790,810,820,830,840,850,860,870,880,890,900,750,760,770,780,790,810,820,830,840,850,860,870,880,890,900,760,770,780,
790,810,820,830,840,850,860,870,880,890,900,760,770,780,790,810,820,830,840,850,860,870,880,890,900,770,780,790,810,820,830,840,850,860,870,880,890,900,780,790,810,820,
830,840,850,860,870,880,890,900,810,820,830,840,850,860,870,880,890,900,820,830,840,850,860,870,880,890,900,830,840,850,860,870,880,890,900,840,850,860,870,880,890,900,
850,860,870,880,890,900,860,870,880,890,900,870,880,890,900,880,890,900,890,900,900}};
...
{	int i,j,k,jj,dd, exists,gr, r ; 		//gr:ニューロングループ指定用変数
	...
	for (i=Ne[gr][num];i<N;i++)	for (j=0;j<M;j++) s[gr][i][j]=-5.0; // 181015	yokusei　-1　to　-10	    抑制性シナプス荷重
    ...
    for(i=0;i<N;i++){
     for(j=0;j<interM;j++){				
       do{
        ...
        else      interpost_s[gr][i][j]=-5;
	interpost_sd[gr][i][j]=0;					//11/08追加
   	interpost_delays[gr][i][j]=getrandom(21)+9.0;			//伝達遅れの初期化: 10~30の間　　　190119　10.0->9.0
      }//jに関するforループ
    }//iに関するforループ
...
/////////////////////////////////////////////////////////////////////////////////////////// 	
	...
	for(num=0/*190107*/;num<NUM_EI_PATTERN;num++){ 
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	sprintf(str,"mkdir %s",st);
	system(str);
	sprintf(str4,"mv trial* %s",st);
 for(trial=0;trial<num_trial;trial++){   //trial文
	...
	sprintf(str2,"mkdir trial%d",trial+1);	
	system(str2);
	sprintf(str3,"mv data_* trial%d",trial+1);
	...
	initialize();					// 関数呼び出し: assign connections, weights, etc. 
		...
		if(sec>=SIM_TIME-10){
                               firing_count[num][gr][i]+=1;    //終わり１０秒だけの発火数をカウント(17'11/7) <=を>=に 1215
                }
                v[gr][i]=-65.0;
		...
		//if(gr==Ngr-1){		//190120　　if　to　gr　tuika
		  // for(gr=0;gr<Ngr;gr++){
...
//////////////////////// updating inter_sd  /////////////////////////////////////////////////////////////////////////////////////////////////////
		    ...
		    for(i=0;i<N;i++){
		       if(v[gr][i]==-65){
                        ...
                        *inter_sd_pre[gr][i][j]+=inter_LTP[grtoogr(gr)][interI_pre[gr][i][j]][t+D+10-(interD_pre[gr][i][j]+1)];
                       }
	            }
		  }
...
////	グループ内のsd更新の後、グループ間のsd更新		///	
		if(sec>=SELF_SPIKE_TIME){
			  ...
			  for(gr=0;gr<Ngr;gr++){
				...
				while(t-firings[/*grtoogr(*/gr/*)*/][z[/*grtoogr(*/gr/*)*/]][0]<10) z[/*grtoogr(*/gr/*)*/]--;//190112　kokode10ijou
  				while(t-firings[/*grtoogr(*/gr/*)*/][z[/*grtoogr(*/gr/*)*/]][0] <D+10+1 && z[/*grtoogr(*/gr/*)*/]!=0)  		                                                                                    
				{
					   ...
					   for(j=0; j< interM; j++)
    					   {
                                               ...
                                               if(t-firings[/*grtoogr(*/gr/*)*/][z[/*grtoogr(*/gr/*)*/]][0]==interpost_delays[/*grtoogr(*/gr/*)*/][firings[/*grtoogr(*/gr/*)*/][z[/*grtoogr(*/gr/*)*/]][1]][j]+1)
                                     		 {i=interpost[/*grtoogr(*/gr/*)*/][firings[/*grtoogr(*/gr/*)*/][z[/*grtoogr(*/gr/*)*/]][1]][j];	//グループgrの発火したニューロンfir..[1]のsyn_j先///shsei   
					            I[/*gr*/grtoogr(gr)][i]+=interpost_s[/*grtoogr(*/gr/*)*/][firings[/*grtoogr(*/gr/*)*/][z[/*grtoogr(*/gr/*)*/]][1]][j];	//current must be  

						  if(firings[/*grtoogr(*/gr/*)*/][z[/*grtoogr(*/gr/*)*/]][1]<Ne[/*grtoogr(*/gr/*)*/][num])		//
					    	     interpost_sd[/*grtoogr(*/gr/*)*/][firings[/*grtoogr(*/gr/*)*/][z[/*grtoogr(*/gr/*)*/]][1]][j]-=inter_LTD[grtoogr(gr)/*gr*/][i];     
                                                 }//if
...
//                      if(gr==Ngr-1){  // I wo tukaunode 2tutomo no I ga sorowanto akan　　　　//20190120
//                       for(gr=0;gr<Ngr;gr++)
...
//////////////////////////////////////////////////////////
if(sec<100||sec>=1900){
         sprintf(fname_LAP_1,"LAP_1_%d_%d_%d_%dsec.txt",EI_1,EI_2,-5,SIM_TIME);
	 sprintf(fname_LAP_2,"LAP_2_%d_%d_%d_%dsec.txt",EI_1,EI_2,-5,SIM_TIME);
		   ...
		   if(gr==0){
		    ...
		    fclose(fs_2);
			   ...
			   LTP[gr][i][j]=LTP[gr][i][1000+j];
                     ...
                     while(1000-firings[gr][z[gr]][0]<D+10/* 181109 D->D+10  181126 ->D　190113　->D+10+1　*/) z[gr]--;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
         ...
         if(sec==0||sec==1100||sec==1500||sec==1999){
       
           sprintf(fname_interpost_s_1,"interpost_s_%d_%dsec_%d_%d_%d.txt",1,sec,EI_1,EI_2,-5);
	   sprintf(fname_interpost_s_2,"interpost_s_%d_%dsec_%d_%d_%d.txt",2,sec,EI_1,EI_2,-5);
           sprintf(fname_s_1,"s_%d_%dsec_%d_%d_%d.txt",1,sec,EI_1,EI_2,-5);
	   sprintf(fname_s_2,"s_%d_%dsec_%d_%d_%d.txt",2,sec,EI_1,EI_2,-5);
